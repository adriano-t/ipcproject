...
{   
	...
	// Fifo
  	//=======================
  	if (mknod("myfifo", S_IFIFO | S_IRUSR | S_IWUSR, 0) == -1)
...
void wait_child(int i)
{
	...
	if (read (fifo , &temp_result, sizeof(struct result) ) == -1)
...
void parent()
{
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
		...
		if (write (fifo , &temp_operation, sizeof(struct operation) ) == -1)
...
void child()
{
    ...
    while(true)
    {
		...
		if (read (fifo , &temp_operation, sizeof(struct operation) ) == -1)
			syserr_ext ("fifo", "read" , __LINE__);
		...
		if (write (fifo , &temp_result, sizeof(struct result) ) == -1)
  			syserr_ext ("fifo", "write" , __LINE__);
