...
{   
	...
	// Shared memory
  	//=======================
	current_operation = (struct operation*) xmalloc(SMKEY_OP, sizeof(struct operation));	
	current_result = (struct result*) xmalloc(SMKEY_RES, sizeof(struct result));  
    free_child = (bool*) malloc(sizeof (bool) * childs_count);	 
    for(i = 0; i < childs_count; i++)
    	free_child[i] = true;
    	
	childs_started = (int*) xmalloc(SMKEY_STARTED, sizeof(int));
	*childs_started = 0;	
...
void parent()
{
	...
	log_msg("[Parent] aspetta l'ultimo figlio");
 	sem_p(sem_parent, WAIT_LAST_CHILD);
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
		...
		//se il figlio è occupato lo attende
		if(!free_child[i])
			wait_child(i);
...
void child()
{
    ...
    // wait the last child to unlock the parent 
  	sem_p(sem_parent, MUTEX); 
    (*childs_started)++;
    
    if(*childs_started == childs_count)
    {
		log_msg_ext("[Child] sblocca il padre", id_number);
    	sem_v(sem_parent, WAIT_LAST_CHILD); 
    }
    sem_v(sem_parent, MUTEX);
